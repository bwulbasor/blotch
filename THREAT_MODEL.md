...
This document states what blotch protects, against whom, and — just as
...
| Encryption passphrase / key | Provided by the user at runtime | Never stored by blotch |
...
- **Residual re-identification risk** from quasi-identifiers ("the 47-year-old CEO
  ...
  (`reidrisk.py`, `blotch risk`) — it is **not** a guarantee of anonymity. True
  k-anonymity requires a population model blotch does not have.
...
- blotch refuses to write an unencrypted vault unless the user explicitly opts
  in (`--allow-plaintext`).
...
## What blotch does NOT protect against
...
- **The daemon on a public interface.** `blotch serve` binds to 127.0.0.1 and
